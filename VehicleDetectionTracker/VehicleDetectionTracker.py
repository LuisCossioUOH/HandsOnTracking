...
class VehicleDetectionTracker:
    ...
    def _encode_image_base64(self, image:np.ndarray):
        """
    ...
    def _decode_image_base64(self, image_base64:np.ndarray):
        """
    ...
    def _convert_meters_per_second_to_kmph(self, meters_per_second:float):
    ...
    def process_frame(self, frame, frame_timestamp, verbose=True):
        """
        ...
        results = self.model.track(self._increase_brightness(frame), persist=True, verbose=verbose,
    ...
    def process_folder(self, folder_path, result_callback):
        """
        Process a folder with images by calling a callback for each frame's results.

        Args:
            folder_path (str): Path to the video file.
            result_callback (function): A callback function to handle the processing results for each frame.
        """
        # Process a video frame by frame, calling a callback with the results.
        images = [name for name in sorted(os.listdir(folder_path))]
        path_images = [os.path.join(folder_path, image) for image in images]
